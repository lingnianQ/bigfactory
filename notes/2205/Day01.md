...
1. 分享Spring Cloud Alibaba 解决方案中常用组件的应用。
2. 分享服务冗余、服务拆分、注册和配置、限流降级、超时重试等服务治理思想。
3. 对秒杀活动的领域建模(DDD)中核心域(活动、场次)、支撑域（商品、库存）、通用域（会员）设计。
4. 推进高并发、高性能、高可用 、高扩展、高安全、低成本、易维护的实现方案。
5. 编写网络接入、客户端、应用服务端、数据库端、负载均衡端进行的一些性能优化手段。
