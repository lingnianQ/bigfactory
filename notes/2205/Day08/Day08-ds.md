...
# 术语分析
数据结构是一种组织和存储数据的方式，可以使我们可以更高效地对存储和操作数据。
数据结构属于计算机科学和软件工程的基础。

## 数组
数组是长度固定的一种数据结构，数组中数据都是相同数据类型。支持随机访问。

1. 用作构建其他数据结构的基础，例如数组列表，堆，哈希表
2. 用于不同的排序算法，例如插入排序，快速排序，冒泡排序和合并排序。
...
这五个约束条件保证了红黑树的新增、删除、查找的最坏时间复杂度均为 O(logn)。
