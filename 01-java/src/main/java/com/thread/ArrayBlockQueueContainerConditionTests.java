 ...
 * @param <T>
 */
class ArrayBlockQueueContainerCondition<T>{

    /**基于此数组存储数据*/
    private Object[] array;
    /**基于size记录有效元素个数*/
    private int size;
    public ArrayBlockQueueContainerCondition(){
        this(16);
    }
    public ArrayBlockQueueContainerCondition(int cap){
        this.array=new Object[cap];
    }

    //JAVA中可以基于这个API执行加锁操作(ReentrantLock类似synchronized，只是在api的维度进行加锁)
     ...
     * @param t
     */
    public  void put(T t){//同步锁为调用此方法的当前对象
        reentrantLock.lock();
        try {
            //1.判断容器是否已满,满了则等待
...
public class ArrayBlockQueueContainerConditionTests {
    public static void main(String[] args) {
        ArrayBlockQueueContainerCondition<Integer> container=
                new ArrayBlockQueueContainerCondition<>();
